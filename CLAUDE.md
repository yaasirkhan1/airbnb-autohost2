...
A nightly **10:00 PM ET cron** (`sendCleaningSchedule` in server.js) texts the cleaner + host a Spanish turnover list via OpenPhone — units with a checkout **tomorrow**; same-day turnovers flagged `⚡ URGENTE`. (Moved from 9 PM so the 7 PM vacant-night extension offers land before the list locks.)
...
- The override is **persisted to the mounted volume, merged into that night's 10 PM run, then auto-expires** — it's date-keyed, so it can never affect a future night. Tell the host to set it **before 10 PM ET** for that night.
