...
Steps:
1. Poll GET https://airbnb-autohost2-production.up.railway.app/health every 5 seconds until uptime resets below 30s (indicating a fresh deploy), max 3 minutes. Use Authorization: Bearer $API_SECRET (read the current value from the local .env / Railway env — never hardcode it here)
